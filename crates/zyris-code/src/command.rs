...
pub fn keys(lang: crate::lang::Lang) -> Vec<(&'static str, &'static str)> {
    ...
    match lang {
        Lang::Ko => vec![
            ...
            ("Del", "목록에서 프로젝트 지우기 ‒ 먼저 물어봅니다"),
            ("@", "파일 목록 ‒ 골라서 경로를 넣습니다"),
            ("↑ ↓", "여러 줄 초안에서는 줄 이동, 비었을 때는 보낸 말 되살리기"),
            ("Ctrl+R", "보낸 말 검색 ‒ 치는 대로 좁혀집니다"),
            ("Ctrl+O", "작업 카드 접기∙펴기"),
            ("Ctrl+T", "할 일 목록 펴기∙접기 (활동 줄을 눌러도 됩니다)"),
            ("Ctrl+P", "계획 펴기∙접기 (계획을 기다리는 중일 때)"),
        ...
        Lang::En => vec![
            ...
            ("Del", "Delete a project from the list ‒ it asks first"),
            ("@", "File list ‒ pick one and its path goes in"),
            ("↑ ↓", "Walk a multi-line draft; bring back something you sent when it is empty"),
            ...
            ("Ctrl+P", "Unfold or fold a plan waiting to be approved"),
