...
//!
//! **The third one lives elsewhere in this repo.** Upstream carried a `DeviceGrant` next to these
//! two and a `from_env` that picked between the three; the library-only zyris no longer ships a
//! program layer, so both moved. The device grant is `crate::enroll::AccountGrant`, because it is
//! built on this app's screen — the code is drawn in the TUI rather than printed behind the
//! alternate screen, which is the whole reason a terminal client can enroll without leaking the
//! code past the frame. The order the sources are tried in is [`crate::enroll::source`].
...
impl StaticToken {
    ...
    /// something other than Attacca. Use [`StaticToken::from_env`] if you want the `atk_`
    /// diagnostic.
