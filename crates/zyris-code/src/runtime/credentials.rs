...
//!
//! **The third one lives elsewhere in this repo**: `crate::enroll::DeviceGrant`, because it is
//! built on this app's screen — the code is drawn in the TUI rather than printed behind the
//! alternate screen. The order the sources are tried in is [`crate::enroll::source`].

use async_trait::async_trait;

/// Why a bearer could not be produced, in the three shades the run loop reacts to differently.
///
/// Written out rather than derived: upstream used `thiserror`, but every arm's message is just the
/// string it carries, so the derive would buy a dependency this crate does not otherwise have.
#[derive(Debug)]
pub enum CredentialsError {
    ...
    Fatal(String),
    /// The credential *source* could not be reached — a server that timed out, a file on a mount
...
/// mounted file that was replaced, or a fresh enrollment after a refusal.
#[async_trait]
pub trait Credentials: Send + Sync + 'static {
    ...
    /// different one is already available. The default is `false`: a credential somebody handed
    /// over is theirs to replace, and answering `true` would loop on one that will never work.
...
/// either is a 401 at the upgrade with nothing to suggest what is actually wrong.
...
impl StaticToken {
    /// Trusts the caller: no prefix check, for a node that talks to something other than Attacca.
    /// Use [`StaticToken::from_env`] if you want the prefix diagnostic.
...
mod tests {
    ...
    async fn a_static_credential_is_returned_verbatim_and_never_logged_whole() {
        ...
        assert!(
            ...
            "a handed-over credential is not this program's to forget"
        );
    }
