...
# ── PATH ─────────────────────────────────────────────────────────────────────
# Already reachable? Then there is nothing to edit, and editing anyway is how startup files
# collect the same line four times.
case ":${PATH}:" in
...
# One marker line, so a second run finds its own work instead of appending again.
marker="# added by zyris-code installer"
...
case "$shell_name" in
  bash)
    ...
    for f in "$HOME/.bashrc" "$HOME/.bash_profile"; do
      ...
      if add_to "$f" "$line"; then edited="$edited $f"; fi
    done
    ;;
  zsh)
    rc="${ZDOTDIR:-$HOME}/.zshrc"
    if add_to "$rc" "$line"; then edited="$edited $rc"; fi
    ;;
  fish)
    ...
    if add_to "$conf" "fish_add_path \"$INSTALL_DIR\""; then edited="$edited $conf"; fi
    ;;
  *)
    if add_to "$HOME/.profile" "$line"; then edited="$edited $HOME/.profile"; fi
    ;;
esac
